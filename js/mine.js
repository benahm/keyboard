...
$(document).ready(function() {
    ...
    $(".news").on("click", function() {
        var $selection = $(":not(:has(*),.newsquery,.hightlight,.loading)");
        $selection.css("opacity", "0.5");
        var $q = $(".newsquery");
        $q.show();
...
function getNews(query, callback) {
    query = query || "computer%20science";
    $.ajax({
        ...
        try {
            ...
            $.ajax({
                ...
                if(data.responseData!=null)
                    callback($("<div>" + data.responseData.feed.entries[0].content + "</div>").text());
                else alert(data.responseDetails);
            });
        ...
        alert("Error occurd while quering Google News");
    });
}
